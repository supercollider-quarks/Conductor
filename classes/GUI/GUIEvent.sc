...
// GRAPHIC PRIMITIVES
			...
			vnumerical: { |win, name, cv, rect| var v, origin;
				cv.asArray.do { |cv|
					...
					origin = v.bounds.origin;
						CVSyncValue(cv, NumberBox(v,Rect(0,0,50,20).moveTo(origin.x, origin.y)));
						StaticText(v,Rect(0,0,40,20).moveTo(origin.x, origin.y + 20))
							.string_(name).align_(\center).font_(Font("Futura", 10));
				};		
			},
			...
			vnslider: { |win, name, cv, rect| 
				...
				cv.asArray.do { |cv|
					...
					origin = v.bounds.origin;
						CVSyncInput(cv, Slider(v, sliderRect.moveTo(origin.x, origin.y)));
						CVSyncValue(cv, NumberBox(v,numRect.moveTo(origin.x, origin.y + sliderRect.height)) );
						StaticText(v,labelRect.moveTo(origin.x, origin.y + sliderRect.height + numRect.height))
							.string_(name).align_(\center).font_(Font("Futura", 10));
				};		
			},
